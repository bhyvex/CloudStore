...
int deleteUser(int userId, uint32_t ip, uint16_t port)
{
    int rt = 0;
    ...
    cstore::pb_MSG_MPC_MU_DELETE_USER deleteUser;
    deleteUser.set_uid(userId);

    if (!deleteUser.SerializeToString(&data)) {
        printf("protobuf serialize failed\n");
        return -1;
    }

    msg.cmd = MSG_MPC_MU_DELETE_USER;
    msg.length = data.length();

    rt = sendPacket(sockFd, msg, data.c_str());

    if (-1 == rt) {
        printf("send delete_user failed\nn");
        return -1;
    }
    ...
    if (-1 == rt) {
        printf("read delete_user_ack failed\n");
        return -1;
    }
...
int main(int argc, char *argv[])
{
    ...
    rt = deleteUser(userId, item.module_ip(0), muport);

    if (-1 == rt) {
        printf("delete user %d failed\n", userId);

    } else {
        printf("delete user %d success\n", userId);
    }
