/*
 * @file WriteSmallFiles.cpp
 * @brief Write small files into specified path
 *
 * @version 1.0
...
#define SMALL_FILE_SIZE         10 * 1024
...
ThreadFunc(void *arg)
{
    ...
    for (i = 0; i < targ->nfiles; ++i) {
        ...
        fd = open(filePath.c_str(), O_WRONLY | O_CREAT | O_EXCL,
                  S_IRUSR | S_IWUSR);

        if (fd < 0) {
            fprintf(stderr, "thread %d, create file error, path %s, %s\n",
                    targ->threadId, filePath.c_str(), strerror(errno));
            return (void *) - 1;
        }

        char *buf = new char[SMALL_FILE_SIZE];

	int loops = rand() % 256;
	for (int i = 0; i < loops; ++i) {
	    buf[rand() % SMALL_FILE_SIZE] = rand() % 256;
        }

        nwrite = write(fd, buf, SMALL_FILE_SIZE);

        if (nwrite != SMALL_FILE_SIZE) {
            fprintf(stderr, "thread %d, write file error, path %s, %s\n",
                    targ->threadId, filePath.c_str(), strerror(errno));
            delete [] buf;
            return (void *) - 1;
        }
