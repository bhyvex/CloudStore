/*
 * @file ReadSmallFiles.cpp
 * @brief read small files from specified path
 *
 * @version 1.0
...
ThreadFunc(void *arg)
{
    ...
    for (i = 0; i < targ->nfiles; ++i) {
        ...
        if (fd < 0) {
            fprintf(stderr, "thread %d, open file error, path %s, %s\n",
                    targ->threadId, filePath.c_str(), strerror(errno));
            return (void *) - 1;
        }

        nread = read(fd, buf, SMALL_FILE_SIZE);

        if (nread != SMALL_FILE_SIZE) {
            fprintf(stderr, "thread %d, read file error, path %s, %s\n",
                    targ->threadId, filePath.c_str(), strerror(errno));
            return (void *) - 1;
        }
