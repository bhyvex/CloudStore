 ...
 * @brief delete files in specified path
 *
 * @version 1.0
...
ThreadFunc(void *arg)
{
    ...
    for (i = 0; i < targ->nfiles; ++i) {
        ...
        if (rc < 0) {
            fprintf(stderr, "thread %d, delete file error, path %s, %s\n",
    ...
    // rmdir
    rc = rmdir(basePath.c_str());

    if (rc < 0) {
        fprintf(stderr, "thread %d, rmdir error, path %s, %s\n",
